...
>(({ className, children, checked, ...props }, ref) => (
  <DropdownMenuPrimitive.CheckboxItem
    ref={ref}
    className={cn(
      "relative flex cursor-pointer select-none items-center rounded-sm py-1.5 pl-8 pr-2 text-[16px] outline-none transition-colors data-[disabled]:pointer-events-none data-[disabled]:opacity-50 group",
      className,
    )}
    checked={checked}
    {...props}
  >
    <span
      className={cn(
        "absolute left-2 flex h-3.5 w-3.5 items-center justify-center pt-[0px]",
        "bg-checkboxBg",
        "group-hover:border-primary",
        checked && "bg-checkboxBgActive",
      )}
    >
      <DropdownMenuPrimitive.ItemIndicator>
        <Check className="h-4 w-4 text-white p-1" />
      </DropdownMenuPrimitive.ItemIndicator>
    </span>
    {children}
  </DropdownMenuPrimitive.CheckboxItem>
